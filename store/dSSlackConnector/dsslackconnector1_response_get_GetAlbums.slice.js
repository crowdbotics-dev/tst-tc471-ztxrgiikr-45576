...
const dsslackconnector1_response_get_GetAlbumsSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [dsslackconnector1_get_v1_albums_list.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [dsslackconnector1_get_v1_albums_list.fulfilled]: (state, action) => {
      if (state.api.loading === "pending") {
        state.entities = action.payload
        state.api.loading = "idle"
      }
    },
    [dsslackconnector1_get_v1_albums_list.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
